...
void PointCloudRenderer::init_GL(glm::vec3 position, 
  ...
  // Load Shader
  depth_shader.shader_program = LoadShader("./shaders/point_sprite_positioning.vert",
                              "./shaders/point_sprite_circle.frag");
  depth_shader.pos_location = glGetAttribLocation(depth_shader.shader_program, "pos");
  depth_shader.modelview_location = glGetUniformLocation(depth_shader.shader_program, "modelview");
  depth_shader.projection_location = glGetUniformLocation(depth_shader.shader_program, "projection");
  depth_shader.screenSize_location = glGetUniformLocation(depth_shader.shader_program, "screenSize");
  depth_shader.radius_location = glGetUniformLocation(depth_shader.shader_program, "radius");

  // Set data attributes
  glVertexAttribPointer(depth_shader.pos_location, 3, GL_FLOAT, false, sizeof(LidarReading), 0);

  // Initialize camera;
  camera = new Camera(position, target, up);

  glUseProgram(depth_shader.shader_program);
  glUniformMatrix4fv(depth_shader.modelview_location, 1, GL_FALSE, glm::value_ptr(camera->modelview));
  glUniformMatrix4fv(depth_shader.projection_location, 1, GL_FALSE, glm::value_ptr(camera->projection));
  glUniform1fv(depth_shader.radius_location, 1, &RADIUS);
...
void PointCloudRenderer::set_screen_size(const int& w, const int& h) {
  ...
  glUseProgram(depth_shader.shader_program);
  glUniform2fv(depth_shader.screenSize_location, 1, glm::value_ptr(screensize));
}
